...
export async function uploadSampleRecordsToDB(config, records) {
  logger('processing Strudel Voice samples...');
  const files = (records || []).filter((file) => file?.id && file?.title && file?.blob);
  if (!files.length) {
    logger('No Strudel Voice samples to import', 'error');
    return;
  }

  await new Promise((resolve, reject) => {
    const request = openDB(config, (objectStore, db) => {
      files.forEach((file) => objectStore.put(file));
      objectStore.transaction.oncomplete = () => {
        logger('Strudel Voice samples written successfully');
        db.close();
        resolve();
      };
